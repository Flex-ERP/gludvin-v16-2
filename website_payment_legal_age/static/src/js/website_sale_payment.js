odoo.define('website_payment_legal_age.payment', require => {
    'use strict';
    
    const checkoutForm = require('payment.checkout_form');
    const publicWidget = require('web.public.widget');

    const websiteSalePaymentLegalAgeMixin = {
         ...
         */
        start: function () {
            this.$checkbox_tc = this.$('#checkbox_tc');
            this.$checkbox_la = this.$('#checkbox_la');
        ...
        _adaptConfirmButton: function () {
            if (this.$checkbox_tc.length > 0 || this.$checkbox_la.length > 0) {
                const disabledReasons = this.$submitButton.data('disabled_reasons') || {};
                disabledReasons.tc = !this.$checkbox_tc.prop('checked') || !this.$checkbox_la.prop('checked');
    ...
    checkoutForm.include(Object.assign({}, websiteSalePaymentLegalAgeMixin, {
        events: Object.assign({}, checkoutForm.prototype.events, {
            'change #checkbox_tc': '_onClickTCCheckbox',
            'change #checkbox_la': '_onClickTCCheckbox',
        }),
    ...
    publicWidget.registry.WebsiteSalePayment = publicWidget.Widget.extend(
        Object.assign({}, websiteSalePaymentLegalAgeMixin, {
